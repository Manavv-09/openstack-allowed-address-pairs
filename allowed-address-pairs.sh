...
while IFS=, read -r sr vm loc pairs; do
  vm=$(echo "$vm" | xargs)
  ...
  [[ ${#valid_pairs[@]} -eq 0 ]] && { log "SKIP: VM '$vm' - no valid IPs in CSV"; ((skipped++)); continue; }
  ...
  if [[ -z "$migration_name" ]]; then
    log "SKIP: VM '$vm' - no Migration CR found for this VM name"
    ((skipped++))
    continue
  fi
  ...
  if [[ ! -f "$logfile" ]]; then
    log "SKIP: VM '$vm' - migration log not found yet ($migration_name)"
    ((skipped++))
    continue
  fi
  ...
  if [[ -z "$macs" ]]; then
    log "SKIP: VM '$vm' - no ports found in migration log yet ($migration_name)"
    ((skipped++))
    continue
  fi
  ...
  for mac in $macs; do
    ...
    if [[ -z "$port" ]]; then
      log "SKIP: VM '$vm' - port not found in Neutron yet (mac $mac)"
      ((skipped++))
      continue
    fi
    ...
    if [[ "$all_ips" == "$existing_sorted" ]]; then
      log "SUCCESS: VM '$vm' port $port (mac $mac) - already up to date"
      ((success++))
      continue
    fi
    ...
    if openstack port set --insecure $args "$port" 2>>"$LOG"; then
      log "SUCCESS: VM '$vm' port $port (mac $mac) - applied: $(echo "$all_ips" | tr '\n' ',' | sed 's/,$//')"
      ((success++))
    else
...
log "=== SYNC COMPLETE === Processed: $processed VMs | Success: $success ports | Skipped: $skipped | Errors: $errors"
